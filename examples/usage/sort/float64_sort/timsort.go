// DO NOT EDIT. Generated with goinline -w -package github.com/sasha-s/go-inline/examples/blueprints/timsort --target-package-name=float64_sort -target-dir=float64_sort Value->float64
...
// Sort sorts an slice of Values in ascending order.
func Sort(a []float64,) {
	err := doSort(a)
	if err != nil {
		// Should never happen.
...
// ReverseSort sorts an slice of Values in descending order.
func ReverseSort(a []float64,) {
	if len(a) < 2 {
		return
	}
	Sort(a)
	reverseRangeI(a, 0, len(a))
}

func doSort(a []float64,) (err error) {
	lo := 0
	hi := len(a)
	nRemaining := hi
	...
	if nRemaining < _MIN_MERGE {
		initRunLen, err := countRunAndMakeAscendingI(a, lo, hi)
		if err != nil {
			return err
		}

		return binarySortI(a, lo, hi, lo+initRunLen)
	}
	...
	for {
		// Identify next run
		runLen, err := countRunAndMakeAscendingI(a, lo, hi)
		if err != nil {
			return err
		}
		...
		if runLen < minRun {
			...
			if err = binarySortI(a, lo, lo+force, lo+runLen); err != nil {
				return err
			}
			runLen = force
		}
...
type timSortHandlerI struct {
	 ...
	 */
	tmp	[]float64	// Actual runtime type will be Object[], regardless of T
 ...
 */
func newTimSortI(a []float64,) (t *timSortHandlerI) {
	t = &timSortHandlerI{
		a:		a,
	...
	t.runBase = make([]int, stackLen)
	t.runLen = make([]int, stackLen)
 ...
 */
func binarySortI(a []float64, lo, hi, start int) (err error) {
...
/**
  ...
  *          the specified array
*/
func countRunAndMakeAscendingI(a []float64, lo, hi int) (int, error) {
 ...
 */
func reverseRangeI(a []float64, lo, hi int) {
	hi--
	for lo < hi {
		a[lo], a[hi] = a[hi], a[lo]
		lo++
 ...
 */
func (t *timSortHandlerI) pushRun(runBase, runLen int) {
	t.runBase[t.stackSize] = runBase
	t.runLen[t.stackSize] = runLen
	t.stackSize++
}
 ...
 * entry to the method.
 */
func (t *timSortHandlerI) mergeCollapse() (err error) {
	for t.stackSize > 1 {
		n := t.stackSize - 2
		if n > 0 && t.runLen[n-1] <= t.runLen[n]+t.runLen[n+1] {
			if t.runLen[n-1] < t.runLen[n+1] {
				n--
			}
			if err = t.mergeAt(n); err != nil {
				return
			}
		} else if t.runLen[n] <= t.runLen[n+1] {
			if err = t.mergeAt(n); err != nil {
				return
			}
		} else {
 ...
 */
func (t *timSortHandlerI) mergeForceCollapse() (err error) {
	for t.stackSize > 1 {
		n := t.stackSize - 2
		if n > 0 && t.runLen[n-1] < t.runLen[n+1] {
			n--
		}
		if err = t.mergeAt(n); err != nil {
			return
 ...
 */
func (t *timSortHandlerI) mergeAt(i int) (err error) {
	if t.stackSize < 2 {
	...
	if i != t.stackSize-2 && i != t.stackSize-3 {
	 ...
	 */
	t.runLen[i] = len1 + len2
	if i == t.stackSize-3 {
		t.runBase[i+1] = t.runBase[i+2]
		t.runLen[i+1] = t.runLen[i+2]
	}
	t.stackSize--
	 ...
	 */
	k, err := gallopRightI(t.a[base2], t.a, base1, len1, 0)
	if err != nil {
		return err
	}
	if k < 0 {
	 ...
	 */
	len2, err = gallopLeftI(t.a[base1+len1-1], t.a, base2, len2, len2-1)
	if err != nil {
		return
	}
	if len2 < 0 {
	...
	if len1 <= len2 {
		err = t.mergeLo(base1, len1, base2, len2)
		if err != nil {
			return fmt.Errorf("mergeLo: %v", err)
		}
	} else {
		err = t.mergeHi(base1, len1, base2, len2)
		if err != nil {
 ...
 *    should follow it.
 */
func gallopLeftI(key float64, a []float64, base, len, hint int) (int, error) {
 ...
 * @return the int k,  0 <= k <= n such that a[b + k - 1] <= key < a[b + k]
 */
func gallopRightI(key float64, a []float64, base, len, hint int) (int, error) {
	if len <= 0 || hint < 0 || hint >= len {
		return 0, errors.New(" len > 0 && hint >= 0 && hint < len;")
	}
 ...
 */
func (t *timSortHandlerI) mergeLo(base1, len1, base2, len2 int) (err error) {
	if len1 <= 0 || len2 <= 0 || base1+len1 != base2 {
		return errors.New(" len1 > 0 && len2 > 0 && base1 + len1 == base2")
	}

	// Copy first run into temp array
	a := t.a	// For performance
	tmp := t.ensureCapacity(len1)
...
outer:
	for {
		...
		for {
			...
			count1, err = gallopRightI(a[cursor2], tmp, cursor1, len1, 0)
			if err != nil {
				return
			}
			if count1 != 0 {
			...
			count2, err = gallopLeftI(tmp[cursor1], a, cursor2, len2, 0)
			if err != nil {
				return
			}
			if count2 != 0 {
	...
	t.minGallop = minGallop	// Write back to field
 ...
 * @param len2  length of second run to be merged (must be > 0)
 */
func (t *timSortHandlerI) mergeHi(base1, len1, base2, len2 int) (err error) {
	if len1 <= 0 || len2 <= 0 || base1+len1 != base2 {
		return errors.New("len1 > 0 && len2 > 0 && base1 + len1 == base2;")
	}
...
outer:
	for {
		...
		for {
			...
			if gr, err := gallopRightI(tmp[cursor2], a, base1, len1, len1-1); err == nil {
				count1 = len1 - gr
			} else {
				return err
			}
			...
			if gl, err := gallopLeftI(a[cursor1], tmp, 0, len2, len2-1); err == nil {
				count2 = len2 - gl
			} else {
				return err
			}
 ...
 */
func (t *timSortHandlerI) ensureCapacity(minCapacity int) []float64 {
	if len(t.tmp) < minCapacity {
