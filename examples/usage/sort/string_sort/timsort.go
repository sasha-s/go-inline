// DO NOT EDIT. Generated with goinline -w -package github.com/sasha-s/go-inline/examples/blueprints/timsort --target-package-name=string_sort -target-dir=string_sort Value->string
...
func doSort(a []string,) (err error) {
	lo := 0
	hi := len(a)
	nRemaining := hi
...
type timSortHandlerI struct {
	 ...
	 */
	tmp	[]string	// Actual runtime type will be Object[], regardless of T
 ...
 */
func newTimSortI(a []string,) (t *timSortHandlerI) {
	t = &timSortHandlerI{
		a:		a,
 ...
 */
func binarySortI(a []string, lo, hi, start int) (err error) {
...
/**
  ...
  *          the specified array
*/
func countRunAndMakeAscendingI(a []string, lo, hi int) (int, error) {
 ...
 */
func reverseRangeI(a []string, lo, hi int) {
	hi--
	for lo < hi {
		a[lo], a[hi] = a[hi], a[lo]
		lo++
 ...
 *    should follow it.
 */
func gallopLeftI(key string, a []string, base, len, hint int) (int, error) {
 ...
 * @return the int k,  0 <= k <= n such that a[b + k - 1] <= key < a[b + k]
 */
func gallopRightI(key string, a []string, base, len, hint int) (int, error) {
	if len <= 0 || hint < 0 || hint >= len {
		return 0, errors.New(" len > 0 && hint >= 0 && hint < len;")
	}
 ...
 */
func (t *timSortHandlerI) ensureCapacity(minCapacity int) []string {
