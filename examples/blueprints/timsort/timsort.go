...
type Value int // Can be anything comparable and ordered. See http://golang.org/ref/spec#Comparison_operators.
...
type timSortHandlerI struct {
	 ...
	 */
	tmp []Value // Actual runtime type will be Object[], regardless of T
 ...
 */
func newTimSortI(a []Value) (t *timSortHandlerI) {
	t = &timSortHandlerI{
		a:         a,
 ...
 */
func binarySortI(a []Value, lo, hi, start int) (err error) {
...
/**
  ...
  *          the specified array
*/
func countRunAndMakeAscendingI(a []Value, lo, hi int) (int, error) {
 ...
 */
func reverseRangeI(a []Value, lo, hi int) {
	hi--
	for lo < hi {
		a[lo], a[hi] = a[hi], a[lo]
		lo++
 ...
 *    should follow it.
 */
func gallopLeftI(key Value, a []Value, base, len, hint int) (int, error) {
 ...
 * @return the int k,  0 <= k <= n such that a[b + k - 1] <= key < a[b + k]
 */
func gallopRightI(key Value, a []Value, base, len, hint int) (int, error) {
	if len <= 0 || hint < 0 || hint >= len {
		return 0, errors.New(" len > 0 && hint >= 0 && hint < len;")
	}
 ...
 */
func (t *timSortHandlerI) ensureCapacity(minCapacity int) []Value {
